...
var Set = wire.NewSet(
	New,
	iio.Provider,
	wire.Bind(new(script.Executor), new(Git)),
)

func (g *Git) Exec(ctx context.Context, opts ...script.Option) (out []byte, err error) {
	var wg sync.WaitGroup

	c := script.BuildCommand("git", opts)
...
func (g *Git) exec(c *script.Command, cmd *exec.Cmd) (out []byte, err error) {
	if c.IOConnected {
		var (
			buf bytes.Buffer
			wg  sync.WaitGroup
...
func (d *Git) Run(ctx context.Context, opts ...script.Option) error {
	var wg sync.WaitGroup
...
func (d *Git) run(c *script.Command, cmd *exec.Cmd) error {
	var (
		buf bytes.Buffer
		wg  sync.WaitGroup
	)
